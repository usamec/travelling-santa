...
inline string UnescapeShit(const string& x) {
  string ret = "";
  char last = 'x';
...
inline string StripTags(const string& x) {
  bool intag = false;
  string ret = "";
  for (int i = 0; i < x.length(); i++) {
    if (x[i] == '<') intag = true;
...
template<class T>
inline void VectorToSet(const vector<T>&a, set<T>* b) {
  for (int i = 0; i < a.size(); i++)
    b->insert(a[i]);
}

template<class T, class T2>
inline void SetToVector(const set<T>&a, vector<T2>* b) {
  for (auto it = a.begin(); it != a.end(); ++it)
    b->push_back(make_pair(*it, 0));
}

template<class TIt, class TType>
inline void SortBySecond(TIt start, TIt end, vector<TType>* out, double limit = 0) {
  out->clear();
...
template<class T>
inline double CalcAP(const vector<T>& rec_items, const set<T>& good_items) {
...
template<class T>
inline double CalcSimilarity(const vector<T>&a, const vector<T>& b) {
  unordered_set<T> uni;
  int inter_size = 0;
  for (int i = 0; i < a.size(); i++)
    uni.insert(a[i]);
...
template<class T>
inline double CalcSimilarityDice(const vector<T>&a, const vector<T>& b, double beta) {
  unordered_set<T> uni;
  int inter_size = 0;
  for (int i = 0; i < a.size(); i++)
    uni.insert(a[i]);
...
template<class T>
inline double CalcSimilarityAToB(const vector<T>&a, const vector<T>& b) {
  unordered_set<T> bs;
  int inter_size = 0;
  for (int i = 0; i < b.size(); i++)
    bs.insert(b[i]);
...
template<class T>
inline double CalcSimilarityBToA(const vector<T>&a, const vector<T>& b) {
  unordered_set<T> bs;
  int inter_size = 0;
  for (int i = 0; i < a.size(); i++)
    bs.insert(a[i]);
...
inline double CalcJaccardsSimilarity(const set<string>&a, const set<string>& b) {
  set<string> uni;
  int inter_size = 0;
...
inline double CalcEarthDist(pair<double, double> a, pair<double, double> b) {
...
inline double CalcLen(pair<double, double> a) {
  return sqrt(a.first*a.first + a.second*a.second);
}

inline double CalcDist(pair<double, double> a, pair<double, double> b) {
  return CalcLen(make_pair(a.first - b.first, a.second - b.second));
}

inline pair<double, double> NormalizePair(pair<double, double> x) {
  double l = CalcLen(x);
  return make_pair(x.first / l, x.second / l);
}

inline void StringToBigrams(const string& in, vector<string>* out) {
  vector<string> words;
...
inline void RemoveFromVector(vector<int>& x, int pos, vector<int>& backids) {
